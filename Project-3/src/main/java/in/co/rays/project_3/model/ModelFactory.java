...
public final class ModelFactory {
	...
	public ProductModelInt getProductModel() {
		...
		if (productModel == null) {
			...
			// If no model is found in cache it creates a new model based on which database
			// is defined in system.properties file.

			if ("Hibernate".equals(DATABASE)) {
				productModel = new ProductModelHibImp();
			}
			modelCache.put("productModel", productModel); // Store created object in cache
		}
		return productModel;
	}
